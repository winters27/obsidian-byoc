...
import { YANDEXDISK_CLIENT_ID, YANDEXDISK_CLIENT_SECRET } from "./baseTypes";
import {
...
class YandexDiskAuthModal extends Modal {
  ...
  onOpen() {
    ...
    const t = this.t;
    const missing = findMissingCredentials({ YANDEXDISK_CLIENT_ID, YANDEXDISK_CLIENT_SECRET });
    if (missing.length > 0) {
      renderUnconfiguredProvider(contentEl, "Yandex Disk", missing);
      return;
    }
