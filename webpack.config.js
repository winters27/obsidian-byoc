...
module.exports = {
  ...
  plugins: [
    ...
    // Resolve node:xxx protocol imports (webpack 5 compatibility)
    ...
    rules: [
      ...
      },
      {
        ...
        use: {
          loader: "ts-loader",
          options: {
            // Skip type checking in webpack — handled separately by tsc.
            // This allows main.js to emit despite pre-existing upstream type errors.
            transpileOnly: true,
          },
        },
  ...
  resolve: {
    ...
    // Map node:xxx protocol imports to browser polyfills
    alias: {
      "node:url": require.resolve("url/"),
      // aggregate-error uses node:url (fileURLToPath) — not browser-safe.
      // Replace with our minimal browser-compatible stub.
      "aggregate-error": path.resolve(__dirname, "src/aggregateErrorStub.ts"),
    },
    fallback: {
